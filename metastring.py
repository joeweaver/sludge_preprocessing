...
# The string may optionally begin with a date and/or time, terminated with an _ if there is additional metadata
#  2019-08-25-23-31 ->  {'date': '2019-08-25', 'time': '23:31'}
#  2019-08-25 -> {'date': '2019-08-25'}
#  23-31  -> {'time': '23:31'}
#
#  If the time pattern is more than 4 digits, split into pairs by a hyphen, the string is kept 'literal' &  the time section is gauranteed to return all text up until the first underscore or period
#  this is intended to allow arbitrary precision and the inclusion of time zone specifiers, etc
#
#  Up to the first period, the string shall then consist of key value pairs, with a hyphen separating keys from values
#  and an underscore separating pairs from each other
#
#  key1-value1_key2-value2 -> {'key1': 'value1', 'key2': 'value2'}
#
#  All text after the first period is ignored. This is to allow non-metadata information in the file.
#
...
#
# You also cannot have multiple values for one key, in effect, this means that you cannot have '-' in a value.
# The parser does not validate values from the metadata.  E.g. It will happilly allow hours > 24, non sequential replicates, etc
